describe('Login E2E', () => {
  ...
  it('1.1.2. login exitoso del usuario(Proveedor) y renderizado del Home Page', () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      cy.get('[data-testid="submit-button-main-form"]').click();
      cy.get('.p-toast-message-content').first().within(() => {
        cy.contains('Error').should('be.visible');
        cy.contains(`Credenciales incorrectas. Intentos restantes: ${2 - i}`).should('be.visible');
      });
    ...
    cy.contains('¿Te llegó el correo de restablecimiento de contraseña?').should('be.visible');
  ...
  it('1.2.1. Muestra error al mandar campos vacios', () => {
		...
		cy.get('.p-toast-message-content').within(() => {
			...
			cy.contains('Debe llenar todos los campos del formulario').should('be.visible');
		});
	});
  //TODO: Sad path login(empty field: email)
  it('1.2.2. Muestra error al mandar un email inexistente', () => {
		cy.visit('http://localhost:3000/login');
