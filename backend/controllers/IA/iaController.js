...
exports.analyzeDocument = (req, res) => {
	...
	exec(
		`python ${path.join(__dirname, 'IA/main.py')} ${filePath}`,
		(error, stdout, stderr) => {
			if (error) {
			...
			const newInvoice = new Invoice({
				...
				total: 100.0, // Este valor puede ser calculado
			...
			newInvoice
				...
				.then(invoice =>
					res
						.status(201)
						.json({ message: 'Factura procesada y guardada', invoice }),
				)
				.catch(err =>
					res
						.status(500)
						.json({ error: 'Error guardando la factura en la base de datos' }),
				);
		},
	);
};
